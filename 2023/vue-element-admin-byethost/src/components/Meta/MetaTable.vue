  ...
  <section v-loading="loading">
    <el-card data-role="main">
      <el-table :data="table.data" size="small" border height="calc(100vh - 265px)" @selection-change="handleTableSelectionChange">
        ...
        <el-table-column v-if="showColumns.includes('children')" align="center" show-overflow-tooltip label="子集数" width="80">
          <template slot-scope="{row,$index}">
            <el-link type="primary" :underline="false" @click.stop="handleTableClickChildren(row, $index)">
              {{ row.children.length }}
            </el-link>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    getTableData() {
      this.loading = true
      this.request.select_meta_list({ ...this.requestParams, page_size: this.table.page_size }, {
    ...
    handleTableClickChildren(row, $index) {
      this.$emit('click-children', { row, $index })
    }
  }
}
</script>
