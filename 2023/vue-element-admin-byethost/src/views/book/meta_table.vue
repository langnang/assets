<template>
  <div class="app-container book book-meta book-meta-table">
...
import { deleteBookMetaList, selectBookMetaList } from '@/api/book'
export default {
  name: 'BookMetaTable',
  components: {
    MetaTableVue
  ...
  data() {
    return {
      ...
      request: {
        select_list: selectBookMetaList
      },
      requestParams: {
        type: ''
      }
  ...
  methods: {
    handleCreate() {
      switch (this.$route.meta.type) {
        case 'category':
          return this.$router.push('/book/create-category')
        case 'tag':
          return this.$router.push('/book/create-tag')
        default: break
      }
    },
    handleDelete() {
      this.$confirm('此操作将永久删除所选数据, 是否继续?', '提示', {
        ...
        deleteBookMetaList({ mids }).then(res => {
    ...
    handleUpdate() {
      ...
      switch (this.$route.meta.type) {
        case 'category':
          return this.$router.push('/book/update-category/' + meta.mid)
        case 'tag':
          return this.$router.push('/book/update-tag/' + meta.mid)
        default: break
      }
    }
  }
