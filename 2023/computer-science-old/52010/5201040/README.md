...
时间复杂度是描述算法运行的时间。我们把算法需要运算的次数用输入大小为 $ n $ 的函数来表示，计作 $ T(n) $
...
| 线性阶   | $ O(n) $       | $ 2n+3 $                | -        |
| 平方阶   | $ O(n^{2}) $   | $ 3^{2}+2n+1 $          | -        |
| 对数阶   | $ O(log{n}) $  | $ 5log_{2}n+20 $        | -        |
| nlogn 阶 | $ O(nlog{n}) $ | $ 2n+3nlog_{2}n+19 $    | -        |
| 立方阶   | $ O(n^{3}) $   | $ 6n^{3}+3n^{2}+4n+10 $ | -        |
| 指数阶   | $ O(2^{n}) $   | $ 2^{n} $               | -        |
| 阶乘     | $ O(n!) $      | $ n! $                  | -        |
...
$ S(n)=O(f(n)) $ ，其中 $ n) 为问题的规模，$ S(n) $ 表示空间
复杂度。通常用 $ S(n) $ 来定义。
