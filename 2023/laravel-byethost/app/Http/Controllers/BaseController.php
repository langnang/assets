  ...
  public function import_tree(Request $request, $global_default = [])
  {
    ...
    try {
        ...
        if ($request->filled($key)) {
          ...
          switch ($type) {
            ...
            case 'object':
              ...
              if ($return[$key] instanceof Exception) throw $return[$key];
              break;
            case 'array':
              $data = $request->input($key, []);
              $data = new Request(["data" => array_map(function ($item) use ($default, $global_default) {
      ...
      if ($request->filled('contents')) {
        ...
        if ($upserted_content_list instanceof Exception) throw $upserted_content_list;
        array_push($_logs, [
