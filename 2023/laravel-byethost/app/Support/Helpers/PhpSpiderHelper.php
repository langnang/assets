...
class PhpSpiderHelper
{
        ...
        if ($queue_lsize > self::$taskid + 2) {
          ...
          if (self::$use_redis and !empty(self::$configs)) {
            if ($configs_active = queue::get('configs_' . self::$configs['name'])) {
              self::$configs = json_decode((string)$configs_active, true);
            }
          }
          // 抓取页面
          $this->collect_page();
  ...
  public function collect_page()
  {
    ...
    // 将爬取到的页面内容暂存至本地
    \Illuminate\Support\Facades\Storage::put('collected/' . md5($url), $html);
    // var_dump($html);
    ...
    if ($is_find_url) {
      // 获取页面原始数据
      // $this->get_html_fields($page['raw'], $url, $page, $this->get_fields([["name" => "original_title", "selector" => "//head//title"]], $page['raw'], $url, $page));
  ...
  public function request_url($url, $link = array())
  {
    ...
    $method = trim(strtolower($link['method'] ?? 'GET'));
    ...
    if ($http_code != 200) {
      ...
      if ($http_code == 301 || $http_code == 302) {
        ...
        if (!empty(self::$configs['max_try']) and in_array($http_code, ['407', '0', '502', '503', '429'])) {
          ...
          echo ("Failed to download page {$url}, retry({$link['try_num']})") . PHP_EOL;
        } else {
          // var_dump($url);
          echo ("Failed to download page {$url}") . PHP_EOL;
          self::$collect_fail++;
        }
  ...
  public function get_urls($html, $collect_url, $depth = 0)
  {
    ...
    foreach ($urls as $key => $url) {
      ...
      $urls[$key] = str_replace(array('"', "'", '&amp;'), array('', '', '&'), (string)$url);
    }
  ...
  public function fill_url($url, $collect_url)
  {
    ...
    // 排除没有被解析成功的语言标签
    if (substr($url, 0, 3) == '<%=' or substr($url, 0, 1) == '{' or substr($url, 0, 2) == ' {') // if(substr($url, 0, 3) == '<%=')
    {
      return false;
    }
  ...
  public function get_html_fields($html, $url, $page, $fields = null)
  {
    ...
    if (!empty($fields)) {
      ...
      if (isset($fields) && is_array($fields)) {
        ...
        // 如果设置了导出选项
        // var_dump($fields);
        if (isset(self::$configs['export']['unique_column'])) $fields[self::$configs['export']['unique_column'] ?? 'slug'] = $page['url'];
        // if (isset(self::$configs['export']['created_at_column'])) $fields[self::$configs['export']['created_at_column']] = date("Y-m-d H:i:s", time());
        // if (isset(self::$configs['export']['spider_original_url_column'])) $fields[self::$configs['export']['spider_original_url_column']] = $page['url'];
        if (!empty(self::$configs['export'])) {
          self::$export_type = isset(self::$configs['export']['type']) ? self::$configs['export']['type'] : '';
          // var_dump(['function' => __FUNCTION__, 'export_type' => self::$export_type]);
          if (self::$export_type == 'csv') {
            ...
            // var_dump("self::\$export_type == 'db'");
            $row = db::get_one("SELECT * FROM `" . self::$export_table . "` WHERE `" . (self::$configs['export']['unique_column'] ?? 'slug') . "` = '" . $url . "'");
            // var_dump(['row', $row]);
            if (empty($row)) {
              // var_dump(['row-insert', $row]);
              db::insert(self::$export_table, $fields);
              // var_dump(['row-insert', $row]);
            } else {
              // var_dump(['row-update', $row]);
              db::update(self::$export_table, $fields, ["`" . (self::$configs['export']['unique_column'] ?? 'slug') . "` = '" . $url . "'"]);
              // var_dump(['row-update', $row]);
              // var_dump(['affect_rows' => $affect_rows]);
            }
            // db::insert(self::$export_table, $fields);
   ...
   * @created time :2016-09-23 17:13
   */
  public function get_fields($confs, $html, $url, $page, $parent = [])
  {
    $fields = array();

    // 检测是否存在original_title

    if (count(array_filter($confs, function ($item) {
      // return $item['name'] == 'original_title';
    })) == 0) {
      // array_push($confs, ['name' => 'original_title', 'selector' => "//head//title"]);
    }
    // dump(["foreach confs", $confs]);
    foreach ($confs as $conf_index => $conf) {
      // 当前field抽取到的内容是否是有多项
      ...
      if (!empty($conf['selector'])) {
        ...
        if (!isset($conf['selector_type']) || $conf['selector_type'] == 'xpath') {
          // 改造：获取父元素html
          if (!empty($parent) && substr($conf['selector'], 0, 2) !== '//') {
            // dump(implode('/', array_slice(explode('/', $conf['selector']), 1)));
            // 去除多种语法造成的异常，如'./','//','/@','(/'
            $conf['selector'] = implode('/', array_slice(explode('/', $conf['selector']), 1));
            $parent_html = $parent['html'];
            $conf['selector'] = "(" . $parent['selector'] . ")" . "[" . $parent['position'] . "]/" . $conf['selector'];
            //
            $conf['selector'] = str_replace('{{parent_index}}', $parent['position'], $conf['selector']);
            $conf['selector'] = str_replace('{{child_index}}', $conf_index + 1, $conf['selector']);
            // dump([$conf_index, $conf, $parent]);
            // dump(["conf_index" => $conf_index, "conf" => $conf, "selector" => $conf['selector'], "parent" =>  $parent, "parent_html" => $parent_html]);
          } else {
            $parent_html = null;
          }
          // 如果找不到，返回的是false
          $values = $this->get_fields_xpath($parent_html ?? $html, $conf['selector'], $conf['name']);
          // dump($values);
        ...
        if (isset($values) && !empty($conf['children'])) {
          ...
          $child_values = array();
          // 父项抽取到的html作为子项的提取内容
          foreach ($values as $child_index => $child_html) {

            // 递归调用本方法, 所以多少子项目都支持
            $child_value = $this->get_fields($conf['children'], $child_html, $url, $page, array_merge($conf, ["html" => $parent_html ?? $html, "position" => $child_index + 1]));
            if (!empty($child_value)) {
              $child_values[] = $child_value;
            }
            // dump([$child_index, $child_html, $conf['children'], $child_value]);
          }
          // exit;
      ...
      if (!empty($conf['filter'])) {
        $fields[$conf['name']] = $this->filter_field($fields[$conf['name']], $conf['filter'], ['html' => $html, "url" => $url]);
      }
    }
   ...
   * * @return void
   * */
  public function filter_field($values, $filters, $vars)
  {
    // dump([$values, $filters, $vars]);
    $filters = str_replace(array_map(function ($key) {
      return "{{{$key}}}";
    }, array_keys($vars)), array_values($vars), $filters);
    if (empty($values)) return $values;
    $_logs = [__METHOD__, $values, "explode filters($filters)"];
    $filters = explode('|', $filters);
    array_push($_logs, $filters);
    // dump($_logs);
    foreach ($filters as $filter) {
      ...
      $latestStr = substr($filter, $endTag === false ? 0 : $endTag + 1);
      // 函数
      ...
      foreach ($args as $index => $arg) {
        ...
        if ($startTag !== false && $endTag !== false) {
          $args[$index] = $this->filter_field($values, $arg, $vars);
        }
      }
      ...
      if (function_exists($func)) {
        if (in_array($func, ['array_search', 'array_map', 'explode'])) {
          // 第二个参数是变量
          $values = call_user_func($func, $args[0], $values, ...array_slice($args, 1));
        } else if (in_array($func, ['str_replace'])) {
          // 第三个参数是变量
          $values = call_user_func($func, $args[0], $args[1] ?? "", $values, ...array_slice($args, 2));
          // dump($values);
        } else if (in_array($func, ['array_keys', 'array_values'])) {
          // 只需要唯一参数
          $values = call_user_func($func, $values);
        } else {
          // 第一个参数是变量
  ...
  public function add_server_list($serverid, $tasknum)
  {
    ...
    if (!$server_list_json) {
      ...
      $server_list = json_decode((string)$server_list_json, true);
      $server_list[$serverid] = array(
        'serverid' => $serverid,
        'tasknum' => $tasknum,
        'time' => time(),
  ...
  public function queue_rpop()
  {
    if (self::$use_redis) {
      ...
      $link = json_decode((string)$link, true);
    } else {
  ...
  public function display_task_ui()
  {
    ...
    foreach ((array)$task_status as $json) {
  ...
  public function display_collect_ui()
  {
    ...
    $display_str .= str_pad((string)$collect, 16);
    $display_str .= str_pad((string)$queue, 14);
    $display_str .= str_pad((string)$collected, 15);
    $display_str .= str_pad((string)$fields, 15);
    $display_str .= str_pad((string)$depth, 12);
    $display_str .= "\n";
    return $display_str;
  }
   ...
   * 爬虫单元测试
   */
  function unit($url = null)
  {
    $_logs = [__METHOD__];
    if (empty($url)) $url = self::$configs['scan_urls'][0];
    $_md5 = md5($url);
    // var_dump($_md5);
    $path = 'collected/' . $_md5 . ".html";
    $exists = \Illuminate\Support\Facades\Storage::exists($path);
    array_push($_logs, $exists ? "file exists." : "file not exists.");
    $html = '';
    // 2 小时内读取缓存文件
    if ($exists && \Illuminate\Support\Facades\Storage::size($path) > 1000) {
      $lastModified = \Illuminate\Support\Facades\Storage::lastModified($path);
      if (time() - $lastModified < 2 * 60 * 60) {
        array_push($_logs, "file last modified at " . date('Y-m-d H:i:s', $lastModified) . ", in 2 hours.");
        array_push($_logs, "get file content.");
        $html = \Illuminate\Support\Facades\Storage::get($path);
      } else {
        array_push($_logs, "file last modified at " . date('Y-m-d H:i:s', $lastModified) . ", over 2 hours.");
      }
    }
    if (empty($html)) {
      array_push($_logs, "request url content.");
      $parse_url = parse_url($url);
      // dump($parse_url);
      $html = $this->request_url($url, ['headers' => [
        "Referer" => "{$parse_url['scheme']}://{$parse_url['host']}/",
        "Host" => $parse_url['host'],
      ]]);
      array_push($_logs, "put file content.");
      \Illuminate\Support\Facades\Storage::put('collected/' . $_md5  . ".html", $html);
    }
    // dump($url);
    array_push($_logs, "get html fields.");
    $return = $this->get_html_fields($html, $url, []);
    array_push($_logs, "put html fields.");
    \Illuminate\Support\Facades\Storage::put('fields/' . $_md5  . '.json', json_encode($return, JSON_UNESCAPED_UNICODE));
    $return["_html"] = $html;
    $return["_url"] = $url;
    $return["_md5"] = $_md5;
    $return["_logs"] = $_logs;
  ...
  function extract_source($url, $fields)
  {
  }
}
