  ...
  <script>
    ...
    */
    ...
    Puzzle.prototype.next = function() {
      ...
      */
      ...
      for (ky = 0; ky <= ny; ++ky) {
        corners[ky] = [];
        for (kx = 0; kx <= nx; ++kx) {
          corners[ky][kx] = new Point((kx + alea(-coeffDecentr, coeffDecentr)) * dx, (ky + alea(-coeffDecentr,
            coeffDecentr)) * dy);
      ...
      for (kp = 0; kp < this.polyPieces.length; kp++) {
        for (kn = 0; kn < this.polyPieces[kp].pieces.length; kn++) {
          ...
          switch (this.freeSpace) {
            case 0:
              pc.pTarget = new Point(this.reqWidth - (2.25 + Math.random() / 4) * this.dx, Math.random() * (this
                .height - this.dy) - this.dy);
              break;
            case 1:
              pc.pTarget = new Point(Math.random() * (this.width - this.dx) - this.dx, this.reqHeight - (2.25 + Math
                .random() / 4) * this.dy);
          } // switch
        } // for kn
      } // for kp
    ...
    Puzzle.prototype.animate = function() {
      ...
      for (kp = 0; kp < this.polyPieces.length; kp++) {
        for (kn = 0; kn < this.polyPieces[kp].pieces.length; kn++) {
          ...
          act = pc.where();
          cib = pc.pTarget;
          pc.moveTo(new Point((this.anim.cpt * act.x + cib.x) / (this.anim.cpt + 1), (this.anim.cpt * act.y + cib.y) / (
            this.anim.cpt + 1)));
          if (this.anim.cpt == 0) {
            delete pc.pTarget;
          }
    ...
    Puzzle.prototype.mouseMoveGame = function(event) {
      ...
      if (x < 2) x = 2;
      if (x > Math.floor(parseFloat(this.divBoard.style.width)) - 2) x = Math.floor(parseFloat(this.divBoard.style
        .width)) - 2;
      if (y < 2) y = 2;
      if (y > Math.floor(parseFloat(this.divBoard.style.height)) - 2) y = Math.floor(parseFloat(this.divBoard.style
        .height)) - 2;
