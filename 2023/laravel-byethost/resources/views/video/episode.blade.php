    ...
    <div class="row row-cols-1">
      <div class="col mb-3">
        <div class="alert alert-secondary mb-0" role="alert">
          <h4 class="alert-heading">{{ $episode['title'] ?? '' }}</h4>
        </div>
      </div>
      <div class="col mb-3">
        @switch($episode['play_type']??'')
          @case('iframe')
            <iframe src="{{ $episode['play_url'] }}" frameborder="0" width="100%" height="100%"></iframe>
          @break

          @default
            <video id="my-player" class="video-js vjs-default-skin vjs-big-play-centered vjs-16-9" preload="auto" autoplay
              controls data-setup='{"fluid": true}'>
              <source src="{{ $episode['play_url'] }}" type="application/x-mpegURL">
            </video>
        @endswitch
      </div>
    </div>
