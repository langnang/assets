...
function select_typecho_user($data, $db = array())
{
  ...
  // 根据令牌查询所属用户信息
  if (!array_key_exists("uids", $_data)) {
    $user = new TypechoUserController($_data, $_conn, $_db);
    $user->select(array("authCode" =>  array_key_exists("token", $_data) ? $_data["token"] : $token));
    return $user;
  }
  // 根据用户ID查询
  else if (array_key_exists("uids", $_data) && sizeof($_data["uids"]) === 0) {
    return $data["user"];
  }
