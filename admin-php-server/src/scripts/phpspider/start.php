...
require_once __DIR__ . "/../../controllers/phpspider/PhpSpider.php";

// var_dump($env_config["__cloud__"]);


$db_config = $env_config["__cloud__"];
...
// $env =  $argv[4];
// $server =  $argv[5];
$info_url = $server . "/api/phpspider/info";
$update_url = $server . "/api/phpspider/update";
...
$default_options["db_config"] = $db_config;
...
$controller = new PhpSpiderContentController();
...
$spider->on_start = function ($phpspider) use ($update_url, $id) {
  // 更新状态
  Requests::post($update_url, array("Content-Type" => "application/json"), json_encode(array("id" => $id, "status" => "running")));
};
...
// @param $page['raw'] 当前网页的内容
// @param $page['request'] 当前网页的请求对象
$spider->on_extract_page = function ($page, $data) use ($id, $controller) {
  // var_dump($page);
  // var_dump($data);
  $title = selector::select($page['raw'], "//head//title");
  $title = trim(is_array($title) ? $title[0] : $title);

  $old_content = $controller->select(new PhpSpiderContentModel(array(
    "phpspider" => $id,
    "url" => $page["url"],
    "title" => $title,
  )));
  ...
  if (sizeof($old_content) === 0) {
    $controller->insert(new PhpSpiderContentModel(array_merge($new_content, array(
      "create_time" => date('Y-m-d H:i:s'),
    ))));
  } else {
    $old_content = $old_content[0];
    $controller->update(
      new PhpSpiderContentModel(array("id" => $old_content["id"])),
      new PhpSpiderContentModel(array_merge($old_content, $new_content))
    );
  }
